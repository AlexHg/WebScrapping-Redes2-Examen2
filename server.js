...
app.get('/scrape',function(req, res){
    ...
    name = uriAnalizer.parse(req.query.url).host+parseInt(Math.random() * 100 + 1 )
    dir = dir+name;
    res.type("text/html");   // Response type as HTML document
    
    scrapping = () => {
        if(usedThread < totalThread){
            usedThread++;
                ...
                .then(($) => { 
                    ...
                    $('a').each( function () {
                        ...
                        if( uri.host == host && !pageTitle.includes(".jpg") && !pageTitle.includes(".png") && !link.includes("?") && !link.includes("#") && !link.includes("tel:")){
                            ...
                            console.log("El sitio ["+link+"] no es valido");
                        }
                        
                    });
    
                    if(!pageTitle.includes(".jpg") && !pageTitle.includes(".png"))
